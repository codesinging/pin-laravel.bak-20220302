...
class AdminPermissionSeederTest extends TestCase
{
    ...
    public function testRoutePermission()
    {
        ...
        self::assertNotNull(AdminPermission::query()->where('name', PermissionName::fromRoute($route))->first());
    }

    public function testMenuPermission()
    {
        $menu = AdminMenu::query()->first();
        self::assertNotNull(AdminPermission::query()->where('name', PermissionName::fromMenu($menu))->first());
    }
}
